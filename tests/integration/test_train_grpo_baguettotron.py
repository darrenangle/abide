"""Integration tests for the Baguettotron GRPO training helpers."""
...
from scripts import train_grpo_baguettotron
...
class _StubForm:
    ...
    def verify(self, _poem: str) -> SimpleNamespace:
        return SimpleNamespace(score=self._score, passed=self._score >= 0.5)
...
def test_create_reward_function_uses_exact_form_metadata() -> None:
    ...
    reward_fn = train_grpo_baguettotron.create_reward_function(forms, use_wandb=False)

    rewards = reward_fn(
        completions=["<think>reasoning</think>This completion is long enough to score."],
        prompts=[[{"role": "user", "content": "Write an Ode poem about winter."}]],
...
def test_create_dataset_preserves_form_name_metadata(monkeypatch) -> None:
    ...
    fake_prompt_generator.LEARNABLE_FORMS = ["Ode"]
    monkeypatch.setitem(sys.modules, "prompt_generator", fake_prompt_generator)

    dataset = train_grpo_baguettotron.create_dataset(num_prompts=1)
