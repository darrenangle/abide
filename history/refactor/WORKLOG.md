...
- 2026-04-10 `RF-084` claimed to add multi-seed mixed-stable validation plus holdout sample export for the modern Gemma 4 E2B path, then run the first recorded validation pass against the RF-083 baseline.
- 2026-04-10 `RF-084` added seed-aware mixed-stable validation and durable holdout export: `scripts/prime_rl_validate.py`, reusable deterministic prompt-record generation, and `ABIDE_SEED` runner passthrough for the modern Gemma 4 E2B path.
- 2026-04-10 `RF-084` ran a two-seed `mixed-stable` validation pass at `models/prime_rl_gemma4_e2b_validation_rf084/`. Seed `42` finished with training reward `0.8833`; seed `43` finished with training reward `1.0000`.
- 2026-04-10 `RF-084` fixed the first live validation bug by changing the temporary evaluation-model overlay to use resolved absolute symlinks and adding `--evaluate-only`, then reran holdout scoring without retraining.
- 2026-04-10 `RF-084` recorded the first durable cross-seed holdout summary at `models/prime_rl_gemma4_e2b_validation_rf084/summary.json`: mean training reward `0.9417`, mean holdout score `0.8071`, mean holdout pass rate `0.375`.
- 2026-04-10 `RF-084` exported scored holdout samples for both seeds and identified the consistent weak forms driving the low pass rate: `Sestina`, `Tanka`, and `Ghazal`, with `Villanelle` still unstable across seeds.
- 2026-04-10 `RF-084` closed and released its lock. Opened `RF-085` to improve mixed-stable generalization against the fixed RF-084 holdout set rather than optimizing on in-run reward alone.
- 2026-04-10 `RF-086` claimed to run a larger modern Gemma 4 E2B `prime-rl` training job on the broader `rl_default` subset with W&B online logging and a heavier rollout budget than the earlier `well_known` baseline.
- 2026-04-11 `RF-086` launched the large `rl_default` Gemma 4 E2B run at `models/prime_rl_gemma4_e2b_rl_default_large_rf086` with W&B online (`rf086-rl-default-large`), using `max_steps=32`, `num_prompts=256`, `batch_size=4`, `rollouts_per_example=4`, `max_tokens=512`, `seq_len=1536`, `learning_rate=5e-6`, and `max_async_level=1`.
- 2026-04-11 `RF-086` directly observed the large run clearing the full startup sequence: inference, orchestrator, and trainer launched; the env server became healthy; W&B synced to `c507437882d145f7b142f8ec57727578`; and the trainer completed Gemma 4 weight load plus initial FSDP setup.
- 2026-04-11 `RF-086` stopped the initial 128-rollout probe intentionally after it reached step `4` with peak GPU memory around `20.4 GiB`, then relaunched the same stable shape at real scale into `models/prime_rl_gemma4_e2b_rl_default_1024_rf086` with W&B run `rf086-rl-default-1024`.
- 2026-04-11 `RF-086` set the active large-run recipe to `max_steps=256`, `num_prompts=256`, `batch_size=4`, `rollouts_per_example=4`, `max_tokens=512`, `seq_len=1536`, `learning_rate=5e-6`, and `max_async_level=1` for `1024` total rollouts, then verified W&B sync to `e89911b6b7174bce869342e3dc3ecd53`.
- 2026-04-11 `RF-087` claimed to build a verifier-gated SFT warmup dataset pipeline for the current training forms, with a reusable library builder plus a JSONL export script.
- 2026-04-11 Closed `RF-087` after landing a verifier-gated SFT warmup dataset pipeline in `abide.training.synthetic_sft`, adding the `build_synthetic_sft_dataset.py` exporter, exporting the helpers through `abide_poetry_forms`, adding focused integration coverage, and writing `data/sft/abide_synthetic_rl_default.jsonl` (`112` rows across the `rl_default` forms).
- 2026-04-11 Claimed `RF-088` to build a hard-form Gemma curriculum: ground a measured difficult-form subset, export a 1000+ row verifier-gated SFT warmup corpus, run a LoRA SFT warmup, and continue RL on that same subset.
- 2026-04-11 Closed `RF-086` after the `rl_default` Gemma 4 E2B `prime-rl` run completed all `256` steps / `1024` rollouts, synced to W&B run `e89911b6b7174bce869342e3dc3ecd53`, and wrote its final artifacts under `models/prime_rl_gemma4_e2b_rl_default_1024_rf086`.
- 2026-04-11 `RF-088` added a codex-spark-authored hard-form warmup loop: generated deterministic task sheets for `Tanka`, `PetrarchanSonnet`, `Villanelle`, `Ghazal`, and `Sestina`, wrote per-form candidate shards with Codex Spark workers, and added validation/export tooling under `src/abide/training/codex_spark_warmup.py` plus `scripts/build_codex_spark_warmup_tasks.py`, `scripts/merge_codex_spark_warmup_candidates.py`, and `scripts/validate_codex_spark_warmup.py`.
- 2026-04-11 `RF-088` tightened the codex-spark acceptance gate after the first Petrarchan shard passed structure but paraphrased Milton and the second shard passed originality but collapsed into near-duplicate templates. The validator now rejects rows too close to same-form seeds and too close to already accepted same-form peers, with focused regression coverage in `tests/integration/test_codex_spark_warmup.py`.
- 2026-04-11 `RF-088` exported the first verified hard-form SFT shard at `data/sft/codex_spark_hard_form_validated.jsonl`: `125` chat-SFT rows (`25` per form across `5` difficult forms), `0` validation failures, `max_seed_similarity=0.5918`, and `max_peer_similarity=0.8030`.
- 2026-04-11 Claimed `RF-089` to extend the Codex Spark corpus path from the hard subset to the full exported Abide form surface and produce a first validated all-forms artifact.
- 2026-04-11 `RF-089` generalized the Codex Spark task builder with `all_forms` and `brief_only` modes, generated `141` one-per-form tasks for the entire exported catalog surface, and ran the writing pass in shards. The original `gpt-5.3-codex-spark` workers hit a provider usage limit mid-batch, so the surface pass finished on fallback subagents.
- 2026-04-11 `RF-089` surfaced and fixed two genuine code blockers during all-forms validation: `CombinedChallenge` catalog defaults were impossible at `num_words=4` with the inherited `20`-vowel target, and `Terzanelle` claimed a 19-line verifier while its rhyme-scheme child still expected 17 lines.
- 2026-04-11 Closed `RF-089` after exporting `data/sft/codex_spark_full_surface_validated.jsonl` with `141 / 141` accepted rows across every exported form instantiated by `load_form_instances()`, plus summary metadata in `data/sft/codex_spark_full_surface_validated.summary.json`.
- 2026-04-11 Claimed `RF-090` to audit the validated full-surface corpus itself: rerun verifier checks on a representative sample, inspect suspicious rows manually, and record any cases where the dataset passes while the structure is obviously misleading.
- 2026-04-11 Closed `RF-090` after directly re-verifying a representative cross-section of the full-surface corpus and confirming that the pass bits are real against the current verifier implementations, while also recording a substantial tail of structurally valid but low-value rows: all-`a` count-satisfying poems (`PrimeVerse`, `NumericalEcho`, `ModularVerse`), minimal lexical shells (`Mesostic`, `PositionalPoem`, `CombinedChallenge`), and heavily templated long forms (`Sestina`, `CrownOfSonnets`, `ChantRoyal`, `Villanelle`). The audit also recorded corpus-level degeneracy signals including repeated exact lines and very low unique-token ratios.
- 2026-04-11 Opened `RF-091` to add anti-degeneracy heuristics and regenerate or filter the weakest validated rows in the full-surface Codex-authored corpus before treating it as broad warmup data.
- 2026-04-11 Claimed `RF-091` and rewired the Codex-authored warmup pipeline around richer prompt/response pairs plus iterative retry support: warmup prompts now explicitly ask for an actual poem rather than a bare structural shell, validation can reject degenerate-but-valid drafts, and the repo now builds retry task sheets that include the rejected draft and concrete validator feedback for the next generation pass.
- 2026-04-11 Claimed `RF-092` to add a second-layer poemness / anti-gaming judge beside the structural verifier, using the smallest available subagent model in this environment as the first cheap judge implementation and recording disagreements where structure passes but the judge rejects.
- 2026-04-11 `RF-092` landed the first durable judge-task flow in `abide.training.poemness_judge`, plus builder/merge scripts and focused tests. The first run used `gpt-5.4-mini` worker subagents to judge a `58`-row batch (`38` heuristic rejects + `20` accepted controls), producing `45` judge rejects, `13` judge accepts, and a `17`-row disagreement set.
- 2026-04-11 `RF-091` folded the small-model results back into corpus regeneration by building `data/sft/codex_spark_full_surface_adjudicated_retry_tasks.jsonl` with `45` retry tasks from the merged structural and model-judge decisions.
- 2026-04-11 `RF-091` ran the full adjudicated retry loop: regenerated all `45` retry tasks in shards, validated the merged batch cleanly, and re-judged the rewrites with `gpt-5.4-mini`. The first rewrite pass improved the batch to `34 / 45` judge accepts.
- 2026-04-11 `RF-091` then ran a second targeted cleanup on the remaining `11` judge rejects. Only `4 / 11` improved under the judge, yielding a final curated artifact `data/sft/codex_spark_full_surface_retry_curated_poemlike.jsonl` with `38` structurally valid, judge-accepted rows and a residual reject file for `7` stubborn long/repetition-heavy forms.
- 2026-04-14 `RF-091` started the form-specific replacement path with `Abecedarian`: added targeted writer guidance requiring real opening words and a coherent scene, added two curated synthetic abecedarian seed poems to `synthetic_sft`, and exported `data/sft/abecedarian_synthetic_curated.jsonl` with `12` clean prompt/response rows built from those seeds.
- 2026-04-14 `RF-091` standardized the synthetic and rollout prompt builders so they now consistently carry all three pieces: a creative seed (`topic` / `tone` / framing), an explicit “write an actual poem” expectation, and the structural brief or restriction text that the model is expected to satisfy.
- 2026-04-12 Claimed `RF-093` to harden `Abecedarian` after confirming the verifier still accepted junk lines like `B quiet field rests` by matching only the first alphabetic character instead of the first real word.
- 2026-04-12 Closed `RF-093` after changing `Abecedarian` to inspect the first alphabetic word on each line, reject single-letter leading tokens except `A` and `I`, preserve punctuation-tolerant openings, and add a regression for the `B quiet field rests` exploit.
- 2026-04-15 `RF-091` finished the last-mile full-surface warmup coverage: added hand-audited manual seeds for `Ballade`, `Canzone`, `CurtalSonnet`, `RondeauRedouble`, and `Rondel`, revalidated them against the live catalog plus anti-degeneracy heuristics, and rebuilt the supplemental seed layer to `136` covered full-surface forms.
- 2026-04-15 `RF-091` also fixed a real verifier hole in `RondeauRedouble` by wiring its stanza-count / stanza-size checks into the composite gate and adding a regression that rejects flattened single-block layouts.
- 2026-04-15 Closed `RF-091` after recomputing combined warmup coverage to `141 / 141` exported forms and materializing the dedicated full-surface SFT artifact `data/sft/abide_warmup_full_surface-0001.jsonl` through `data/sft/abide_warmup_full_surface-0006.jsonl` (`1337` prompt/response rows total, `1313` synthetic + `24` public-domain).
